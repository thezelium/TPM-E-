...
const PATCH_BUILD_VERSION = "s3-v2-patched-07";
...
function importDataToStorage(data) {
  return new Promise((resolve, reject) => {
    ...
    const forceArraySettingKeys = new Set([
      ...
      "TM_useFolderList",
      "TM_useDeletedChatIDs",
      "TM_useDeletedPromptIDs",
      "TM_useDeletedCharacterIDs",
      "TM_useDeletedPluginIDs",
      "TM_useDeletedFolderIDs",
      "TM_usePinnedCharacterOrder",
    ]);
    ...
    const buildDynamicFallbackKeyCandidates = (attachmentId, indexedEntryMap) => {
      if (!attachmentId) {
        return [];
      }
      const collected = [];
      indexedEntryMap.forEach((entryValue, entryKey) => {
        if (!entryValue) {
          return;
        }
        if (!entryKey.includes(attachmentId)) {
          return;
        }
        ...
        if (
          ...
          lowerKey.startsWith("client_cache_") ||
          lowerKey.startsWith("attachment_")
        ) {
          collected.push(entryKey);
        }
      });
      return collected.slice(0, 60);
    };
    const resolveReferencedCacheKeys = (rawValue, indexedEntryMap) => {
      ...
      directKeys.add(trimmed);
      const uuidMatch = trimmed.match(UUID_PATTERN);
      if (uuidMatch) {
        const id = uuidMatch[0];
        buildAttachmentFallbackKeyCandidates(id).forEach((key) =>
          directKeys.add(key)
        );
        directKeys.add(`BLOB_${id}`);
      }
      const resolved = [];
      directKeys.forEach((candidateKey) => {
        if (indexedEntryMap.has(candidateKey)) {
          resolved.push({
            key: candidateKey,
            value: indexedEntryMap.get(candidateKey),
          });
        }
      });
      return resolved;
    };
    const buildRecoveryCandidatesFromFallbackEntry = (
      fallbackKey,
      fallbackRawValue,
      indexedEntryMap
    ) => {
      ...
      pushCandidate("root", fallbackRawValue);
      if (fallbackRawValue && typeof fallbackRawValue === "object") {
        [
          "value",
          "data",
          "blob",
          ...
          if (Object.prototype.hasOwnProperty.call(fallbackRawValue, fieldName)) {
            pushCandidate(fieldName, fallbackRawValue[fieldName]);
          }
        });
        [
          "blobKey",
          ...
          if (
            Object.prototype.hasOwnProperty.call(fallbackRawValue, fieldName) &&
            typeof fallbackRawValue[fieldName] === "string"
          ) {
            resolveReferencedCacheKeys(
              fallbackRawValue[fieldName],
              indexedEntryMap
            ).forEach((resolvedEntry) => {
              pushCandidate(`${fieldName}->${resolvedEntry.key}`, resolvedEntry.value);
            });
          }
        });
      }
      resolveReferencedCacheKeys(fallbackKey, indexedEntryMap).forEach(
        (resolvedEntry) => {
          pushCandidate(`fallbackKey->${resolvedEntry.key}`, resolvedEntry.value);
        }
      );
      return candidates;
    };
            ...
            if (!preserveKeys.includes(preserveCheckKey)) {
              try {
                ...
                if (isBlobStorageKey && !isBlobLikeValue(value)) {
                  ...
                  const fallbackKeySet = new Set([
                    ...buildAttachmentFallbackKeyCandidates(attachmentId),
                    ...buildDynamicFallbackKeyCandidates(
                      attachmentId,
                      indexedEntryMap
                    ),
                  ]);
                  const fallbackKeys = Array.from(fallbackKeySet).slice(0, 80);
                  for (let i = 0; i < fallbackKeys.length; i++) {
                    ...
                    const recoveryCandidates =
                      buildRecoveryCandidatesFromFallbackEntry(
                        fallbackKey,
                        fallbackRawValue,
                        indexedEntryMap
                      );
                    for (let j = 0; j < recoveryCandidates.length; j++) {
                      const candidate = recoveryCandidates[j];
                      const candidateValue = parseMaybeJsonString(
                        candidate.value,
                        `${fallbackKey}.${candidate.label}`
                      );
                      ...
                      if (isBlobLikeValue(recoveredBlob)) {
                        ...
                        logToConsole(
                          "info",
                          `Recovered ${preserveCheckKey} from ${fallbackKey} (${candidate.label})`
                        );
                        break;
                      }
                    }
