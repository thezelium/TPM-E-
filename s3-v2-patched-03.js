...
const PATCH_BUILD_VERSION = "s3-v2-patched-03";
    ...
    if (data.localStorage) {
      Object.entries(data.localStorage).forEach(([key, settingData]) => {
        if (!preserveKeys.includes(key) && !blockedLocalStorageKeys.has(key)) {
          try {
            const value =
              isStorageSourceWrapper(settingData)
                ? settingData.data
                : settingData;
            const source =
              isStorageSourceWrapper(settingData) && settingData.source
                ? settingData.source
                : "localStorage";
            if (source === "indexeddb") {
              return;
              ...
              try {
                ...
                if (
                  ...
                  try {
                    ...
                    logToConsole(
                      "warning",
                      `Failed to parse ${preserveCheckKey} as JSON, using as-is`,
                      parseError
                    );
                  }
                }
                value = reviveBinaryValue(value, [preserveCheckKey]);
                if (isBlobStorageKey && !isBlobLikeValue(value)) {
                  const fallbackBytes =
                    toUint8FromUnknownBinaryPayload(value) ||
                    findBinaryPayloadDeep(value);
                  if (fallbackBytes) {
                    value = createBinaryFileOrBlob(fallbackBytes, {
                      mimeType: getMimeTypeFromBinaryCarrier(value),
                      name:
