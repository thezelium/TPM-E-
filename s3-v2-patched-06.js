...
const PATCH_BUILD_VERSION = "s3-v2-patched-06";
  ...
  return path.some(
    (segment) =>
      ...
      (/^BLOB_/i.test(segment) ||
        /^CLIENT_CACHE_attachment-/i.test(segment) ||
...
function reviveBinaryValue(value, path = []) {
  ...
  if (hasLegacyBinaryMarker || (hasLegacyBinaryPayload && hasBinaryHint(path))) {
    const bytes = toUint8FromUnknownBinaryPayload(
      value.base64 || value.data || value.bytes
    );
    if (bytes) {
  ...
  if (hasBinaryHint(path) || isLikelyBlobStorageKey(path)) {
    const bytes = toUint8FromUnknownBinaryPayload(value);
    if (bytes) {
      return createBinaryFileOrBlob(bytes, {
...
function importDataToStorage(data) {
  return new Promise((resolve, reject) => {
    ...
    const reviveForStorage = (rawValue, keyPath) => {
                ...
                if (isBlobStorageKey && !isBlobLikeValue(value)) {
                  ...
                  for (let i = 0; i < fallbackKeys.length; i++) {
                    ...
                    for (let j = 0; j < recoveryCandidates.length; j++) {
                      ...
                      const recoveredBlob = reviveForStorage(candidateValue, [
                        fallbackKey,
                        "fallback",
                        candidate.label,
                        preserveCheckKey,
