...
function toUint8FromUnknownBinaryPayload(payload) {
  ...
  if (typeof payload === "object") {
    ...
    if (payload.bytes !== undefined) {
      return toUint8FromUnknownBinaryPayload(payload.bytes);
    }
    const keys = Object.keys(payload);
    const numericKeys = keys.filter((key) => /^\d+$/.test(key));
    if (numericKeys.length > 0 && numericKeys.length === keys.length) {
      const orderedKeys = numericKeys.map(Number).sort((a, b) => a - b);
      if (
        orderedKeys[0] === 0 &&
        orderedKeys[orderedKeys.length - 1] === orderedKeys.length - 1
      ) {
        const bytes = new Uint8Array(orderedKeys.length);
        for (let i = 0; i < orderedKeys.length; i++) {
          const val = payload[String(orderedKeys[i])];
          if (
            typeof val !== "number" ||
