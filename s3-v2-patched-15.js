// === BLOB PROTECTION SHIELD (must run before any TypingMind native code) ===
(function installBlobProtection() {
  const originalDelete = IDBObjectStore.prototype.delete;
  let protectionActive = true;
  let blockedCount = 0;

  IDBObjectStore.prototype.delete = function(key) {
    if (protectionActive && typeof key === 'string' &&
        (key.startsWith('BLOB_') || key.startsWith('CLIENT_CACHE_'))) {
      blockedCount++;
      console.debug('[v14 Shield] Blocked deletion of:', key);
      // Return a valid IDBRequest by deleting a non-existent key (harmless no-op)
      return originalDelete.call(this, '__NOOP_SHIELD_' + Date.now() + '__');
    }
...
const PATCH_BUILD_VERSION = "s3-v2-patched-15";
...
async function rebuildSettingsMetadataFromCurrentStorage() {
  if (!localMetadata || typeof localMetadata !== "object") {
    localMetadata = {};
  }
  if (!localMetadata.settings || typeof localMetadata.settings !== "object") {
  ...
  const indexedEntries = await new Promise((resolve, reject) => {
  ...
  localMetadata.settings.items = rebuiltItems;
  localMetadata.settings.lastModified = rebuildTimestamp;
  localMetadata.settings.syncedAt = 0;
  logToConsole("info", "Rebuilt settings metadata after restore", {
    indexedDBItems: indexedDBItemCount,
    localStorageItems: localStorageItemCount,
    totalSettingsMetadataItems: Object.keys(rebuiltItems).length,
  });
}
...
const BINARY_PAYLOAD_FIELD_CANDIDATES = [
  "base64",
  "data",
  "bytes",
  "buffer",
...
function byteArrayToUint8(value) {
  if (!Array.isArray(value) || value.length === 0) {
    return null;
  }
  const bytes = new Uint8Array(value.length);
  for (let i = 0; i < value.length; i++) {
    const byte = coerceByteValue(value[i]);
    if (byte === null) {
      return null;
    }
    bytes[i] = byte;
...
function looksLikeByteArray(value) {
  return byteArrayToUint8(value) !== null;
}
const BINARY_HINT_EXCLUDED_SEGMENTS = [
  "localblobid",
  "localfileid",
];
function hasBinaryHint(path) {
  return path.some(
    (segment) =>
      typeof segment === "string" &&
      !BINARY_HINT_EXCLUDED_SEGMENTS.includes(segment.toLowerCase()) &&
      BINARY_KEY_HINTS.some((hint) => segment.toLowerCase().includes(hint))
  );
}
function isLikelyBlobStorageKey(path) {
  return path.some(
    (segment) =>
      typeof segment === "string" &&
      (/^BLOB_/i.test(segment) ||
        /^CLIENT_CACHE_image-/i.test(segment))
  );
}
...
function looksLikeBase64String(value, minLength = 24) {
  if (typeof value !== "string") {
    return false;
  }
...
function base64ToUint8(base64) {
  const normalized = normalizeBase64Input(String(base64 || ""));
  if (!normalized) {
...
function extractNumericKeyedBytes(payload) {
  ...
  const lastIndex = numericKeys[numericKeys.length - 1];
  if (lastIndex !== numericKeys.length - 1) {
    return null;
  }
  const bytes = new Uint8Array(numericKeys.length);
  for (let i = 0; i < numericKeys.length; i++) {
    const value = payload[String(i)];
    const byte = coerceByteValue(value);
    if (byte === null) {
      return null;
    }
    bytes[i] = byte;
...
function toUint8FromUnknownBinaryPayload(payload) {
  if (payload == null) {
    return null;
  }
  ...
  if (Array.isArray(payload) && looksLikeByteArray(payload)) {
    return byteArrayToUint8(payload);
  }
  if (typeof payload === "string") {
    ...
    if (parsedDataUrl) {
      return base64ToUint8(parsedDataUrl.base64);
    }
  ...
  if (typeof payload === "object") {
    if (payload.type === "Buffer" && payload.data !== undefined) {
      const bufferBytes = toUint8FromUnknownBinaryPayload(payload.data);
      if (bufferBytes) {
        return bufferBytes;
      }
    }
    if (Array.isArray(payload.data) && looksLikeByteArray(payload.data)) {
      return byteArrayToUint8(payload.data);
    }
    if (typeof payload.base64 === "string") {
      return toUint8FromUnknownBinaryPayload(payload.base64);
    }
    if (payload.data !== undefined) {
      const bytesFromData = toUint8FromUnknownBinaryPayload(payload.data);
      if (bytesFromData) {
        return bytesFromData;
      }
    }
    if (payload.bytes !== undefined) {
      const bytesFromBytes = toUint8FromUnknownBinaryPayload(payload.bytes);
      if (bytesFromBytes) {
        return bytesFromBytes;
      }
    }
    if (payload.buffer !== undefined) {
      const bytesFromBuffer = toUint8FromUnknownBinaryPayload(payload.buffer);
      if (bytesFromBuffer) {
        return bytesFromBuffer;
      }
    }
    if (payload.content !== undefined) {
      const bytesFromContent = toUint8FromUnknownBinaryPayload(payload.content);
      if (bytesFromContent) {
        return bytesFromContent;
      }
    }
    if (payload.value !== undefined) {
      const bytesFromValue = toUint8FromUnknownBinaryPayload(payload.value);
      if (bytesFromValue) {
        return bytesFromValue;
      }
    }
    const bytesFromNumericKeys = extractNumericKeyedBytes(payload);
    if (bytesFromNumericKeys) {
      return bytesFromNumericKeys;
    }
    for (let i = 0; i < BINARY_PAYLOAD_FIELD_CANDIDATES.length; i++) {
      const fieldName = BINARY_PAYLOAD_FIELD_CANDIDATES[i];
      if (!Object.prototype.hasOwnProperty.call(payload, fieldName)) {
        continue;
      }
      const bytes = toUint8FromUnknownBinaryPayload(payload[fieldName]);
      if (bytes) {
        return bytes;
      }
    }
...
function findBinaryPayloadDeep(payload, depth = 0, seen = new WeakSet()) {
  if (payload == null || depth > 8) {
    return null;
  }
  const direct = toUint8FromUnknownBinaryPayload(payload);
  if (direct) {
    return direct;
  }
  if (typeof payload !== "object") {
  ...
  if (Array.isArray(payload)) {
    for (let i = 0; i < payload.length; i++) {
      const nested = findBinaryPayloadDeep(payload[i], depth + 1, seen);
      if (nested) {
        return nested;
      }
    }
  ...
  const prioritized = [];
  for (let i = 0; i < BINARY_PAYLOAD_FIELD_CANDIDATES.length; i++) {
    const key = BINARY_PAYLOAD_FIELD_CANDIDATES[i];
    if (Object.prototype.hasOwnProperty.call(payload, key)) {
      prioritized.push(key);
    }
  }
  const remaining = keys.filter((key) => !prioritized.includes(key));
  const orderedKeys = prioritized.concat(remaining);
  for (let i = 0; i < orderedKeys.length; i++) {
    const key = orderedKeys[i];
    const nested = findBinaryPayloadDeep(payload[key], depth + 1, seen);
    if (nested) {
      return nested;
    }
  }
...
function extractMimeTypeFromUnknownBinaryPayload(
  value,
  depth = 0,
  seen = new WeakSet()
) {
  ...
  if (typeof value === "string") {
    const parsedDataUrl = parseDataUrl(value.trim());
  ...
  const directMimeCandidates = [
    value.mimeType,
    value.contentType,
    value.mime,
    value.type,
  ];
  for (let i = 0; i < directMimeCandidates.length; i++) {
    const candidate = directMimeCandidates[i];
  ...
  if (Array.isArray(value)) {
    const sampleLength = Math.min(8, value.length);
    for (let i = 0; i < sampleLength; i++) {
      const nestedMime = extractMimeTypeFromUnknownBinaryPayload(
        value[i],
        depth + 1,
        seen
      );
...
function getMimeTypeFromBinaryCarrier(value, fallback = "application/octet-stream") {
  const detected = extractMimeTypeFromUnknownBinaryPayload(value);
  return detected || fallback;
}
function createBinaryFileOrBlob(bytes, options = {}) {
  const mimeType = options.mimeType || "application/octet-stream";
  const hasFileCtor = typeof File !== "undefined";
  const shouldUseFile = !!options.preferFile && hasFileCtor && !!options.name;
  if (shouldUseFile) {
    try {
...
async function serializeValueForBackup(value, seen = new WeakSet()) {
  if (value === undefined) {
    return null;
  }
  if (value === null) {
  ...
  if (value instanceof ArrayBuffer) {
    return {
      __tm_type: "ArrayBuffer",
      base64: uint8ToBase64(new Uint8Array(value)),
    };
  }
  if (value instanceof DataView) {
    return {
      __tm_type: "DataView",
      base64: uint8ToBase64(
  ...
  if (
    ...
    looksLikeByteArray(value.data)
  ) {
    return {
      __tm_type: "Buffer",
      base64: uint8ToBase64(Uint8Array.from(value.data)),
    };
  }
  if (Array.isArray(value)) {
    const arr = [];
    for (let i = 0; i < value.length; i++) {
      arr.push(await serializeValueForBackup(value[i], seen));
      if (i > 0 && i % 25 === 0) {
  ...
  if (typeof value === "object") {
    ...
    for (let i = 0; i < keys.length; i++) {
      const key = keys[i];
      out[key] = await serializeValueForBackup(value[key], seen);
      if (i > 0 && i % 25 === 0) {
        await new Promise((resolve) => setTimeout(resolve, 0));
      }
    }
...
function serializeIndexedDBKey(key) {
  if (key instanceof Date) {
    return { __tm_key_type: "date", value: key.toISOString() };
  }
  if (Array.isArray(key)) {
    return {
      __tm_key_type: "array",
      value: key.map((item) => serializeIndexedDBKey(item)),
    };
  }
  switch (typeof key) {
    case "string":
      return { __tm_key_type: "string", value: key };
    case "number":
      return { __tm_key_type: "number", value: key };
    case "boolean":
      return { __tm_key_type: "boolean", value: key };
    default:
      return { __tm_key_type: "string", value: String(key) };
  }
}
function deserializeIndexedDBKey(serializedKey) {
  if (
    !serializedKey ||
    typeof serializedKey !== "object" ||
    !serializedKey.__tm_key_type
  ) {
    return serializedKey;
  }
  const { __tm_key_type: keyType, value } = serializedKey;
  switch (keyType) {
    case "date":
    ...
    case "array":
      return Array.isArray(value)
        ? value.map((item) => deserializeIndexedDBKey(item))
        : [];
    case "string":
    default:
      return String(value);
...
function reviveTypedArray(subtype, bytes) {
  ...
  const Ctor = constructors[subtype];
  if (!Ctor) {
...
function reviveBinaryValue(value, path = []) {
  const rootPathSegment = typeof path[0] === "string" ? path[0] : "";
  const preserveAttachmentCacheRecord =
    /^CLIENT_CACHE_attachment-/i.test(rootPathSegment);
  const ignoreRootAttachmentKeyHint =
    path.length === 1 && /^CLIENT_CACHE_attachment-/i.test(rootPathSegment);
  const hasBinaryPathHint = hasBinaryHint(path) && !ignoreRootAttachmentKeyHint;
  const shouldAttemptBinaryReconstruction =
    hasBinaryPathHint || isLikelyBlobStorageKey(path);
  if (value === null || value === undefined) {
    return value;
  }
  if (typeof value === "string") {
    const parsedDataUrl = parseDataUrl(value);
    if (parsedDataUrl && hasBinaryPathHint) {
      const bytes = base64ToUint8(parsedDataUrl.base64);
      return createBinaryFileOrBlob(bytes, {
        mimeType: parsedDataUrl.mimeType,
      });
    }
    if (hasBinaryPathHint && looksLikeBase64String(value)) {
      const bytes = toUint8FromUnknownBinaryPayload(value);
      if (bytes) {
        return createBinaryFileOrBlob(bytes, {
  ...
  if (preserveAttachmentCacheRecord) {
    return value;
  }
  if (Array.isArray(value)) {
    if (looksLikeByteArray(value) && value.length > 512 && hasBinaryHint(path)) {
      return createBinaryFileOrBlob(Uint8Array.from(value), {
        mimeType: "application/octet-stream",
      });
    }
    return value.map((item, index) =>
      reviveBinaryValue(item, path.concat(String(index)))
    );
  }
  if (value.__tm_type === "CircularRef") {
    return null;
  }
  if (value.__tm_type === "Date") {
    return new Date(value.value);
  }
  if (
    value.__tm_type === "Blob" ||
    value.__tm_type === "File" ||
    value.__tm_type === "Buffer"
  ) {
    const bytes = toUint8FromUnknownBinaryPayload(
      value.base64 || value.data || value.bytes
    );
    if (bytes) {
      return createBinaryFileOrBlob(bytes, {
        ...
        preferFile: value.__tm_type === "File",
      });
    }
  }
  if (value.__tm_type === "ArrayBuffer") {
    const bytes = toUint8FromUnknownBinaryPayload(value.base64 || value.data);
    if (bytes) {
  ...
  if (value.__tm_type === "TypedArray") {
    const bytes = toUint8FromUnknownBinaryPayload(value.base64 || value.data);
    if (bytes) {
      return reviveTypedArray(value.subtype, bytes);
    }
  }
  const hasLegacyBinaryMarker =
    value.__is_blob === true ||
    value.__is_file === true ||
    value.type === "Buffer";
  const hasLegacyBinaryPayload =
    typeof value.base64 === "string" ||
    typeof value.data === "string" ||
    Array.isArray(value.data) ||
    Array.isArray(value.bytes);
  if (
    hasLegacyBinaryMarker ||
    (hasLegacyBinaryPayload && hasBinaryPathHint)
  ) {
    const bytes = toUint8FromUnknownBinaryPayload(
      value.base64 || value.data || value.bytes
    );
    if (bytes) {
      const payloadFromDataUrl =
        typeof value.data === "string" ? parseDataUrl(value.data) : null;
      return createBinaryFileOrBlob(bytes, {
        mimeType:
          value.mimeType ||
          value.contentType ||
          value.type ||
          (payloadFromDataUrl && payloadFromDataUrl.mimeType) ||
          "application/octet-stream",
        name: value.name || value.fileName,
        lastModified: value.lastModified,
        preferFile: value.__is_file === true || !!value.name || !!value.fileName,
      });
    }
  }
  if (shouldAttemptBinaryReconstruction) {
    const bytes = toUint8FromUnknownBinaryPayload(value);
    if (bytes) {
      return createBinaryFileOrBlob(bytes, {
        mimeType: getMimeTypeFromBinaryCarrier(value),
        name: value && typeof value === "object" ? value.name || value.fileName : undefined,
        lastModified:
          value && typeof value === "object" ? value.lastModified : undefined,
        preferFile:
          !!(
            value &&
            typeof value === "object" &&
  ...
  Object.entries(value).forEach(([key, nestedValue]) => {
    output[key] = reviveBinaryValue(nestedValue, path.concat(key));
  });
  return output;
}
function isStorageSourceWrapper(value) {
  return (
    value &&
    typeof value === "object" &&
    !Array.isArray(value) &&
    ...
    (value.source === "localStorage" || value.source === "indexeddb")
  );
}
      ...
      if (typeof value === "object" && value !== null) {
        if (Array.isArray(value)) {
          ...
          for (let j = 0; j < value.length; j++) {
            ...
            const stringifiedArrayValue = JSON.stringify(arrayValue);
            result +=
              stringifiedArrayValue === undefined
                ? "null"
                : stringifiedArrayValue;
          ...
          for (let j = 0; j < objKeys.length; j++) {
            ...
            const stringifiedObjValue = JSON.stringify(objValue);
            result += `"${objKey}":${
              stringifiedObjValue === undefined ? "null" : stringifiedObjValue
            }`;
            if (result.length > chunkSize) {
              await new Promise((resolve) => setTimeout(resolve, 0));
            }
...
function exportBackupData() {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      logToConsole("warning", "Failed to read localStorage for backup", error);
    }
    const exportData = {
    ...
    request.onsuccess = function (event) {
      ...
      const transaction = db.transaction(["keyval"], "readonly");
      const store = transaction.objectStore("keyval");
      const collectData = new Promise((resolveData, rejectData) => {
        const keysRequest = store.getAllKeys();
        keysRequest.onsuccess = function (keyEvent) {
          const keys = keyEvent.target.result || [];
          logToConsole("info", "IndexedDB keys found", {
            count: keys.length,
          });
          const valuesRequest = store.getAll();
          valuesRequest.onsuccess = async function (valueEvent) {
            try {
              const values = valueEvent.target.result;
              for (let i = 0; i < keys.length; i++) {
                const key = keys[i];
                const serializedValue = await serializeValueForBackup(values[i]);
                exportData.indexedDB[String(key)] = serializedValue;
                exportData.indexedDBEntries.push({
                  key: serializeIndexedDBKey(key),
          ...
          valuesRequest.onerror = () => rejectData(valuesRequest.error);
        };
        keysRequest.onerror = () => rejectData(keysRequest.error);
      });
      Promise.all([
        collectData,
        ...
        .then(() => {
          ...
          const hasIndexedDBData =
            ...
            exportData.indexedDBEntries.length > 0;
          logToConsole("info", "Export data summary", {
            localStorageKeys: Object.keys(exportData.localStorage).length,
            indexedDBKeys:
              exportData.indexedDBEntries.length ||
              Object.keys(exportData.indexedDB).length,
            hasLocalStorageData,
            hasIndexedDBData,
          });
...
async function restoreFromBackup(key) {
  ...
  try {
    ...
    const countChatEntriesInBackup = (content) => {
      if (!content || typeof content !== "object") {
        return 0;
      }
      if (
        Array.isArray(content.indexedDBEntries) &&
        content.indexedDBEntries.length > 0
      ) {
        return content.indexedDBEntries.filter((entry) => {
          const rawKey = deserializeIndexedDBKey(entry?.key);
          return typeof rawKey === "string" && rawKey.startsWith("CHAT_");
        }).length;
      }
      if (content.indexedDB && typeof content.indexedDB === "object") {
        return Object.keys(content.indexedDB).filter((entryKey) =>
          entryKey.startsWith("CHAT_")
        ).length;
      }
      return 0;
    };
    ...
    try {
      ...
      logToConsole("info", "Parsed content structure:", {
        ...
        hasIndexedDB:
          !!parsedContent.indexedDB || !!parsedContent.indexedDBEntries,
        localStorageKeys: parsedContent.localStorage
          ? Object.keys(parsedContent.localStorage).length
          : 0,
        indexedDBKeys:
          parsedContent.indexedDBEntries &&
          Array.isArray(parsedContent.indexedDBEntries)
            ? parsedContent.indexedDBEntries.length
            : parsedContent.indexedDB
      ...
      logToConsole(
        ...
        typeof decryptedContent === "string"
          ...
          : JSON.stringify(decryptedContent).slice(0, 200)
      );
    ...
    const expectedChatCount = countChatEntriesInBackup(parsedContent);
    logToConsole("info", "Importing data to storage...");
    await importDataToStorage(parsedContent);
    const chats = await getAllChatsFromIndexedDB();
    logToConsole("info", "Restore chat verification", {
      expectedChatCount,
      restoredChatCount: chats.length,
    });
    if (expectedChatCount > 0 && chats.length === 0) {
      throw new Error(
        `Restore verification failed: backup contains ${expectedChatCount} CHAT_* records but none were readable after import`
      );
    }
    if (!localMetadata || typeof localMetadata !== "object") {
      localMetadata = {};
    ...
    localMetadata.chats = {};
    for (const chat of chats) {
      const chatId = chat?.id || (typeof chat?.chatID === "string" ? chat.chatID : "");
      if (!chatId) {
    ...
    localMetadata.lastSyncTime = 0;
    await rebuildSettingsMetadataFromCurrentStorage();
...
function importDataToStorage(data) {
  return new Promise((resolve, reject) => {
    ...
    const blockedLocalStorageKeys = new Set([
      "setItem",
      "getItem",
      "removeItem",
      "clear",
    ...
    const extensionUrlSettingKey = "TM_useExtensionURLs";
    const forceArraySettingKeys = new Set([
      "TM_useExtensionURLs",
      "TM_useDraftMCPServersJSON",
      "TM_useInstalledPlugins",
      "TM_useMCPServers",
      "TM_useSelectedMCPServers",
      "TM_useFolderList",
    ]);
    const detectCurrentExtensionScriptUrl = () => {
      try {
        const scriptUrls = Array.from(
          document.querySelectorAll("script[src]")
        )
          .map((scriptEl) => scriptEl.src)
          .filter(Boolean);
        return (
          scriptUrls.find((url) =>
            /s3-v2-patched-\d+\.js|typingmind-cloud-backup\/s3-v2\.js/i.test(
              url
            )
          ) || ""
        );
    ...
    const isBlobLikeValue = (value) => {
      return (
        (typeof Blob !== "undefined" && value instanceof Blob) ||
        (typeof File !== "undefined" && value instanceof File)
      );
    };
    const toStringKey = (key) => (typeof key === "string" ? key : String(key));
    const parseMaybeJsonString = (value, contextKey) => {
      if (
        typeof value === "string" &&
        ...
        try {
          ...
          logToConsole(
            "warning",
            `Failed to parse ${contextKey} as JSON, using as-is`,
            parseError
          );
        }
      }
    ...
    const normalizeArraySettingValue = (value, contextKey) => {
      const parsedValue = parseMaybeJsonString(value, contextKey);
    ...
    const sanitizeExtensionUrlList = (value) => {
      const arr = normalizeArraySettingValue(
        value,
        extensionUrlSettingKey
      ).map((entry) => {
    ...
    const normalizeSourceWrapper = (settingData, defaultSource = "indexeddb") => {
      const wrapped = isStorageSourceWrapper(settingData);
      return {
        source:
          wrapped && settingData.source ? settingData.source : defaultSource,
        value: wrapped ? settingData.data : settingData,
      };
    };
    const currentExtensionScriptUrl = detectCurrentExtensionScriptUrl();
    const preservedExtensionUrls = sanitizeExtensionUrlList(
      localStorage.getItem(extensionUrlSettingKey)
    );
    if (preservedExtensionUrls.length === 0 && currentExtensionScriptUrl) {
      preservedExtensionUrls.push(currentExtensionScriptUrl);
    }
    const toBlobFromBinaryCandidate = (candidate) => {
      const bytes =
        toUint8FromUnknownBinaryPayload(candidate) ||
        findBinaryPayloadDeep(candidate);
      if (!bytes) {
        return null;
      }
      return createBinaryFileOrBlob(bytes, {
        mimeType: getMimeTypeFromBinaryCarrier(candidate),
        name:
        ...
        preferFile:
          !!(
            ...
            (candidate.__is_file === true ||
              candidate.__tm_type === "File" ||
              candidate.name ||
              candidate.fileName)
          ),
      });
    };
    const reviveForStorage = (rawValue, keyPath, options = {}) => {
      const revived = reviveBinaryValue(rawValue, keyPath);
      if (isBlobLikeValue(revived)) {
        return revived;
      }
      if (!options.coerceRootToBlob) {
        return revived;
      }
      const reconstructed = toBlobFromBinaryCandidate(revived);
      return reconstructed || revived;
    };
    const attachmentHintById = new Map();
    const getTextAttachmentHint = (textValue) => {
      if (typeof textValue !== "string" || !textValue.includes("Attachment:")) {
        return { name: "", mimeType: "" };
      }
      const match = textValue.match(
        /\[(?:File|Image)\s+Attachment:\s*(.+?)\s*\(([^)]+)\)\]/i
      );
      return {
        name: match?.[1]?.trim() || "",
    ...
    const mergeAttachmentHint = (attachmentId, nextHint) => {
      if (!attachmentId || typeof attachmentId !== "string") {
        return;
      }
      const existingHint = attachmentHintById.get(attachmentId) || {
        mimeType: "",
        name: "",
      };
      const mergedHint = {
        mimeType: nextHint?.mimeType || existingHint.mimeType || "",
        name: nextHint?.name || existingHint.name || "",
      };
      attachmentHintById.set(attachmentId, mergedHint);
    };
    const normalizeChatAttachmentMessages = (chatValue, chatKey) => {
      if (!chatValue || typeof chatValue !== "object") {
        return 0;
      }
      let normalizedCount = 0;
      const visited = new WeakSet();
      const stack = [chatValue];
      let guard = 0;
      while (stack.length > 0 && guard < 500000) {
        guard++;
        ...
        if (
          (node.type === "tm_blob_file" || node.type === "tm_image_file" || node.type === "tm_audio_file") &&
          ...
          const metadata =
            node.metadata && typeof node.metadata === "object" && !Array.isArray(node.metadata)
              ? node.metadata
              : null;
          const textHint = getTextAttachmentHint(node.text);
          if (!metadata) {
            node.metadata = {};
            normalizedCount++;
          }
          const safeMetadata = node.metadata;
          // --- v13: ensure localBlobID is always set (zombie scanner key field) ---
          if (
            typeof safeMetadata.localBlobID !== "string" ||
            !safeMetadata.localBlobID.trim()
          ) {
            safeMetadata.localBlobID = node.id;
            normalizedCount++;
          }
          // --- v13: ensure MIME type ---
          if (
            (typeof safeMetadata.type !== "string" ||
              !safeMetadata.type.includes("/")) &&
            textHint.mimeType
          ) {
            safeMetadata.type = textHint.mimeType;
            normalizedCount++;
          }
          // --- v13: ensure name ---
          if (typeof safeMetadata.name !== "string" || !safeMetadata.name) {
            const candidateName = textHint.name || "";
            if (candidateName) {
              safeMetadata.name = candidateName;
              normalizedCount++;
            }
          }
          // --- v13: fileFormat (native shape requires this) ---
          if (typeof safeMetadata.fileFormat !== "string" || !safeMetadata.fileFormat) {
            const mimeForFormat = safeMetadata.type || textHint.mimeType || "";
            if (node.type === "tm_image_file" || mimeForFormat.startsWith("image/")) {
              safeMetadata.fileFormat = "image";
            } else if (node.type === "tm_audio_file" || mimeForFormat.startsWith("audio/")) {
              safeMetadata.fileFormat = "audio";
            } else if (mimeForFormat === "application/pdf") {
              safeMetadata.fileFormat = "pdf";
            } else if (mimeForFormat.startsWith("text/") || mimeForFormat === "Unknown Type") {
              safeMetadata.fileFormat = "text";
            } else {
              safeMetadata.fileFormat = "other";
            }
            normalizedCount++;
          }
          // --- v13: size (keep existing if present, else 0) ---
          if (typeof safeMetadata.size !== "number") {
            safeMetadata.size = 0;
            normalizedCount++;
          }
          // --- v13: text field format must match native: "[File Attachment: name (mime)]" ---
          const resolvedName = safeMetadata.name || textHint.name || "attachment";
          const resolvedMime = safeMetadata.type || textHint.mimeType || "application/octet-stream";
          const expectedTextPrefix = "[File Attachment:";
          if (typeof node.text !== "string" || !node.text.includes(expectedTextPrefix)) {
            if (node.type === "tm_image_file") {
              node.text = `[Image Attachment: ${resolvedName} (${resolvedMime})]`;
            } else {
              node.text = `[File Attachment: ${resolvedName} (${resolvedMime})]`;
            }
            normalizedCount++;
          } else if (typeof node.text === "string" && node.text.includes(expectedTextPrefix) && !node.text.includes("(")) {
            // Has "[File Attachment: name]" but lacks "(mime)" - fix it
            node.text = node.text.replace(/\]$/, ` (${resolvedMime})]`);
            normalizedCount++;
          }
          mergeAttachmentHint(node.id, {
            mimeType:
              (typeof safeMetadata.type === "string" &&
                safeMetadata.type.includes("/") &&
                safeMetadata.type) ||
              ...
              "",
            name:
              (typeof safeMetadata.name === "string" && safeMetadata.name) ||
              textHint.name ||
              "",
          });
        }
      ...
      if (guard >= 500000) {
        logToConsole("warning", `Attachment normalization reached scan limit in ${chatKey}`);
      }
      return normalizedCount;
    };
    const normalizeBlobMimeWithHint = (blobLikeValue, attachmentHint) => {
      if (!isBlobLikeValue(blobLikeValue) || !attachmentHint) {
        return blobLikeValue;
      }
      const targetMime = attachmentHint.mimeType;
      if (typeof targetMime !== "string" || !targetMime.includes("/")) {
        return blobLikeValue;
      }
      const currentMime =
        typeof blobLikeValue.type === "string" ? blobLikeValue.type : "";
      if (currentMime && currentMime !== "application/octet-stream") {
        return blobLikeValue;
      }
      try {
        if (typeof File !== "undefined" && blobLikeValue instanceof File) {
          const fallbackName =
            (typeof attachmentHint.name === "string" && attachmentHint.name) ||
            blobLikeValue.name ||
            "attachment";
          return new File([blobLikeValue], fallbackName, {
            type: targetMime,
            lastModified: blobLikeValue.lastModified || Date.now(),
          });
        }
        return new Blob([blobLikeValue], { type: targetMime });
      } catch (_error) {
        return blobLikeValue;
      }
    };
    const getAttachmentIdFromBlobKey = (key) => {
      const match = /^BLOB_(.+)$/i.exec(key);
      return match ? match[1] : null;
    };
    const buildAttachmentFallbackKeyCandidates = (attachmentId) => {
      if (!attachmentId) {
        return [];
      }
      return [
        `CLIENT_CACHE_attachment-${attachmentId}`,
        `CLIENT_CACHE_image-${attachmentId}`,
        `CLIENT_CACHE_file-${attachmentId}`,
        `CLIENT_CACHE_blob-${attachmentId}`,
        `CLIENT_CACHE_${attachmentId}`,
        `ATTACHMENT_${attachmentId}`,
      ];
    };
    const isAttachmentCacheKey = (key) =>
      /^CLIENT_CACHE_(attachment|image|file|blob)-/i.test(key) ||
      /^ATTACHMENT_/i.test(key);
    // --- v13: normalize CLIENT_CACHE value to native shape ---
    const normalizeAttachmentCacheValue = (value, key) => {
      if (
        !isAttachmentCacheKey(key) ||
        !value ||
        typeof value !== "object" ||
        isBlobLikeValue(value)
      ) {
        return value;
      }
      const normalized = { ...value };
      // Ensure cacheGroup exists (native: "attachment_parser")
      if (!normalized.cacheGroup) {
        normalized.cacheGroup = "attachment_parser";
      }
      // Ensure value wrapper exists with native shape: {text, rawText, base64}
      if (normalized.value && typeof normalized.value === "object") {
        const inner = { ...normalized.value };
        // Fix base64 data URI prefix if missing
        if (typeof inner.base64 === "string" && inner.base64.length > 0) {
          if (!inner.base64.startsWith("data:")) {
            // Attempt to determine MIME from context
            const attachmentUuid = key.replace(/^CLIENT_CACHE_(?:attachment|image|file|blob)-/i, "");
            const hint = attachmentHintById.get(attachmentUuid);
            const mime = hint?.mimeType || "application/octet-stream";
            inner.base64 = `data:${mime};base64,${inner.base64}`;
          }
        }
        normalized.value = inner;
      }
      return normalized;
    };
    const refreshAttachmentCacheExpiry = (value, key) => {
      if (
        !isAttachmentCacheKey(key) ||
        !value ||
        typeof value !== "object" ||
      ...
      // --- v13: also apply normalizeAttachmentCacheValue ---
      let refreshed = normalizeAttachmentCacheValue(value, key);
      if (!Object.prototype.hasOwnProperty.call(refreshed, "expireAt")) {
        // Ensure expireAt exists (native shape requires it)
        const oneYearMs = 365 * 24 * 60 * 60 * 1000;
        refreshed.expireAt = Date.now() + oneYearMs;
        logToConsole("info", `Added missing expireAt to cache: ${key}`);
        return refreshed;
      }
      refreshed = { ...refreshed };
      const oneYearMs = 365 * 24 * 60 * 60 * 1000;
      const nextExpiryTimestamp = Date.now() + oneYearMs;
      if (typeof refreshed.expireAt === "number") {
        refreshed.expireAt = nextExpiryTimestamp;
      } else if (typeof refreshed.expireAt === "string") {
        const parsedDate = Date.parse(refreshed.expireAt);
        if (!Number.isNaN(parsedDate)) {
          refreshed.expireAt = new Date(nextExpiryTimestamp).toISOString();
        } else if (/^\d+$/.test(refreshed.expireAt.trim())) {
          refreshed.expireAt = String(nextExpiryTimestamp);
        } else {
          refreshed.expireAt = new Date(nextExpiryTimestamp).toISOString();
        }
      } else {
      ...
      logToConsole("info", `Refreshed attachment cache expiry: ${key}`, {
    ...
    let blobKeyTotal = 0;
    let blobKeyRevived = 0;
    let blobRecoveredFromAttachmentCache = 0;
    let blobMimeRetagged = 0;
    let chatAttachmentMetadataNormalized = 0;
    let arrayNormalizedInLocalStorage = 0;
    let arrayNormalizedInIndexedDB = 0;
    let extensionUrlsPreserved = 0;
    const unresolvedBlobKeySamples = [];
    if (data.localStorage) {
      Object.entries(data.localStorage).forEach(([key, settingData]) => {
        if (!preserveKeys.includes(key) && !blockedLocalStorageKeys.has(key)) {
          try {
            if (key === extensionUrlSettingKey) {
              return;
            }
            const normalizedEntry = normalizeSourceWrapper(
              settingData,
              "localStorage"
            );
            ...
            if (forceArraySettingKeys.has(key)) {
              valueToStore = JSON.stringify(
                normalizeArraySettingValue(valueToStore, key)
              );
              arrayNormalizedInLocalStorage++;
    ...
    if (preservedExtensionUrls.length > 0) {
      localStorage.setItem(
        extensionUrlSettingKey,
        JSON.stringify(preservedExtensionUrls)
      );
      extensionUrlsPreserved++;
    } else {
      localStorage.removeItem(extensionUrlSettingKey);
    }
    if (data.indexedDB || data.indexedDBEntries) {
      ...
      request.onsuccess = function (event) {
        ...
        transaction.oncomplete = () => {
          logToConsole("success", `Settings restore completed`, {
            ...
            blobKeysFound: blobKeyTotal,
            blobKeysRevived: blobKeyRevived,
            blobRecoveredFromAttachmentCache,
            blobMimeRetagged,
            chatAttachmentMetadataNormalized,
            arrayNormalizedInLocalStorage,
            arrayNormalizedInIndexedDB,
            extensionUrlsPreserved,
            unresolvedBlobSamples: unresolvedBlobKeySamples,
            scriptVersion: PATCH_BUILD_VERSION,
        ...
        deleteRequest.onsuccess = function () {
          const indexedEntries =
            Array.isArray(data.indexedDBEntries) &&
            data.indexedDBEntries.length > 0
              ? data.indexedDBEntries.map((entry) => [
                  deserializeIndexedDBKey(entry.key),
                  entry.value,
                ])
              : Object.entries(data.indexedDB || {});
          const keyRestorePriority = (rawKey) => {
            const key = toStringKey(rawKey);
            if (/^CHAT_/i.test(key)) {
              return 0;
            }
            if (/^CLIENT_CACHE_attachment-/i.test(key)) {
              return 1;
            }
            if (/^BLOB_/i.test(key)) {
              return 2;
          ...
          indexedEntries.sort(
            ([aKey], [bKey]) => keyRestorePriority(aKey) - keyRestorePriority(bKey)
          );
          const indexedEntryMap = new Map(
            indexedEntries.map(([key, entryValue]) => [toStringKey(key), entryValue])
          );
          indexedEntries.forEach(([rawKey, settingData]) => {
            const preserveCheckKey = toStringKey(rawKey);
            if (!preserveKeys.includes(preserveCheckKey)) {
              try {
                if (preserveCheckKey === extensionUrlSettingKey) {
                  return;
                }
                const isChatStorageKey = /^CHAT_/i.test(preserveCheckKey);
                const isBlobStorageKey = /^BLOB_/i.test(preserveCheckKey);
                if (isBlobStorageKey) {
                  blobKeyTotal++;
                }
                const normalizedEntry = normalizeSourceWrapper(
                ...
                let value = normalizedEntry.value;
                const source = normalizedEntry.source;
                if (source === "localStorage") {
                  return;
                }
                value = parseMaybeJsonString(value, preserveCheckKey);
                value = reviveForStorage(value, [preserveCheckKey], {
                  coerceRootToBlob: isBlobStorageKey,
                });
                // v15: normalize AFTER revive so localBlobID survives binary revival
                if (isChatStorageKey) {
                  chatAttachmentMetadataNormalized +=
                    normalizeChatAttachmentMessages(value, preserveCheckKey);
                }
                if (isBlobStorageKey && isBlobLikeValue(value)) {
                  const attachmentIdHint =
                    getAttachmentIdFromBlobKey(preserveCheckKey);
                  const normalizedBlob = normalizeBlobMimeWithHint(
                    value,
                    attachmentHintById.get(attachmentIdHint)
                  );
                  if (normalizedBlob !== value) {
                    blobMimeRetagged++;
                ...
                if (isBlobStorageKey && !isBlobLikeValue(value)) {
                  const attachmentId =
                    getAttachmentIdFromBlobKey(preserveCheckKey);
                  const fallbackKeys =
                    buildAttachmentFallbackKeyCandidates(attachmentId);
                  for (let i = 0; i < fallbackKeys.length; i++) {
                    const fallbackKey = fallbackKeys[i];
                    if (!indexedEntryMap.has(fallbackKey)) {
                      continue;
                    }
                    const fallbackEntry = normalizeSourceWrapper(
                      indexedEntryMap.get(fallbackKey),
                      "indexeddb"
                    );
                    if (fallbackEntry.source === "localStorage") {
                      continue;
                    }
                    const fallbackRawValue = parseMaybeJsonString(
                      fallbackEntry.value,
                      fallbackKey
                    );
                    const recoveredBlob = reviveForStorage(
                      fallbackRawValue,
                      [fallbackKey, "fallback", preserveCheckKey],
                      { coerceRootToBlob: true }
                    );
                    if (isBlobLikeValue(recoveredBlob)) {
                      const fallbackHint = {
                        mimeType:
                          attachmentHintById.get(attachmentId)?.mimeType ||
                          getMimeTypeFromBinaryCarrier(fallbackRawValue),
                        name:
                          attachmentHintById.get(attachmentId)?.name || "",
                      };
                      const normalizedRecoveredBlob = normalizeBlobMimeWithHint(
                        recoveredBlob,
                        fallbackHint
                      );
                      if (normalizedRecoveredBlob !== recoveredBlob) {
                        blobMimeRetagged++;
                      }
                      value = normalizedRecoveredBlob;
                      blobRecoveredFromAttachmentCache++;
                      logToConsole(
                        "info",
                        `Recovered ${preserveCheckKey} from ${fallbackKey}`
                      );
                      break;
                    }
                  }
                }
                value = refreshAttachmentCacheExpiry(value, preserveCheckKey);
                if (forceArraySettingKeys.has(preserveCheckKey)) {
                  value = normalizeArraySettingValue(value, preserveCheckKey);
                  arrayNormalizedInIndexedDB++;
                }
                if (isBlobStorageKey && isBlobLikeValue(value)) {
                  blobKeyRevived++;
                  ...
                  isBlobStorageKey &&
                  unresolvedBlobKeySamples.length < 12
                ) {
                  unresolvedBlobKeySamples.push(preserveCheckKey);
                }
                objectStore.put(value, rawKey);
                settingsRestored++;
                logToConsole(
                  "info",
                  `Restored setting to IndexedDB: ${preserveCheckKey}`
                );
              } catch (error) {
                logToConsole(
                  "error",
                  `Error restoring IndexedDB setting ${preserveCheckKey}:`,
                  error
                );
              }
            }
          ...
          if (preservedExtensionUrls.length > 0) {
            objectStore.put(preservedExtensionUrls, extensionUrlSettingKey);
            extensionUrlsPreserved++;
            settingsRestored++;
            logToConsole(
              "info",
              `Preserved extension URLs in IndexedDB: ${extensionUrlSettingKey}`
            );
          }
        };
      };
...
// --- v13: attachment ownership diagnostics (toggle: ?attachmentDiag=true) ---
async function runAttachmentOwnershipDiag() {
  logToConsole("info", "[DIAG] Starting attachment ownership verification...");
  ...
  for (let i = 0; i < allKeys.length; i++) {
    ...
    const m = /^BLOB_(.+)$/i.exec(k);
    if (m) blobIds.add(m[1]);
  }

  // Collect referenced localBlobIDs from all chat/agent/project values
  const referencedIds = new Set();
  for (let i = 0; i < allValues.length; i++) {
    const jsonStr = JSON.stringify(allValues[i]);
    if (!jsonStr.includes("localBlobID")) continue;
    const matches = jsonStr.match(/"localBlobID"\s*:\s*"([^"]+)"/g);
    if (matches) {
      for (const m of matches) {
        const idMatch = /"localBlobID"\s*:\s*"([^"]+)"/.exec(m);
  ...
  for (const blobId of blobIds) {
    if (!referencedIds.has(blobId)) {
      orphanCount++;
      if (orphanSamples.length < 10) {
        orphanSamples.push(blobId);
      }
    }
  }
  const ownedCount = blobIds.size - orphanCount;

  logToConsole(orphanCount > 0 ? "warning" : "success",
    `[DIAG] Ownership check complete`, {
    totalBlobs: blobIds.size,
    owned: ownedCount,
    orphan: orphanCount,
    orphanSamples: orphanSamples,
    referencedIdsTotal: referencedIds.size,
  });

  if (orphanCount > 0) {
    logToConsole("warning",
      `[DIAG] ${orphanCount} blob(s) have NO matching localBlobID reference — these will be deleted as zombies on refresh`
    );
  } else {
    logToConsole("success",
      `[DIAG] All ${blobIds.size} blobs are properly referenced — zombie cleanup should preserve them`
    );
  }
}
      ...
      if (key === "settings.json") {
        if (
          ...
          try {
            ...
            const normalizeArraySettingValue = (value) => {
            ...
            const preservedExtensionUrls = normalizeArraySettingValue(
              localStorage.getItem(extensionUrlSettingKey)
            )
              .map((entry) => {
              ...
              .filter(Boolean);
            if (preservedExtensionUrls.length === 0) {
              const currentScriptUrl = detectCurrentExtensionScriptUrl();
              if (currentScriptUrl) preservedExtensionUrls.push(currentScriptUrl);
            }
            let settingsRestored = 0;
              ...
              if (!preserveKeys.includes(key)) {
                try {
                  ...
                  if (source === "indexeddb") {
                    ...
                    if (forceArraySettingKeys.has(key)) {
                      valueToStore = normalizeArraySettingValue(valueToStore);
                    }
                    ...
                    if (forceArraySettingKeys.has(key)) {
                      localStorage.setItem(
                        key,
                        JSON.stringify(normalizeArraySettingValue(value))
                      );
                    } else {
                      localStorage.setItem(
                        key,
            ...
            if (preservedExtensionUrls.length > 0) {
              ...
              await setIndexedDBKey(
                extensionUrlSettingKey,
                preservedExtensionUrls
              );
              settingsRestored++;
              logToConsole(
                "info",
                `Preserved extension URLs during settings restore: ${extensionUrlSettingKey}`
              );
            }
            if (cloudMetadata.settings) {
              localMetadata.settings.lastModified =
